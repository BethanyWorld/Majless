...
class iPhorm_Element
{
     ...
     *
     * @param   string  $tooltipType  The current type of tooltip
    ...
    public function getLabelHtml($tooltipType, $tooltipEvent, $labelCss, $for = true)
    {
        ...
        $output .= '<label' . ($for ? ' for="' . esc_html($this->getUniqueId()) . '"' : '') . $this->getCss('label', $labelCss) . ' class="' . esc_attr($this->getName()) . '-outer-label">';
        $output .= $label;

        if ($this->getRequired() && strlen($requiredText = $this->_form->getRequiredText()))  {
            $output .= '<span class="iphorm-required">*</span>';
        }

        if ($this->_form->getUseTooltips() && strlen($this->getTooltip()) && $tooltipType == 'icon') {
            $output .= '<span class="iphorm-tooltip-icon iphorm-tooltip-icon-' . $tooltipEvent . '"><span class="iphorm-tooltip-icon-content">' . $this->getTooltip() . '</span></span>';
        }
    ...
    /**
     * Get the placeholder of the element
     *
     * @return string
     */
    ...
    /**
     * Get the value formatted as string
     *
     * @return string
     ...
     *
     * @param $separator The separator for array types
     * @return string
     */
    public function getValueHtml($separator = '<br />')
    {
        ...
        if (is_scalar($filteredValue)) {
            $output = nl2br(esc_html((string) $filteredValue));
