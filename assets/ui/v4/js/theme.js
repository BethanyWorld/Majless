...
$(document).ready(function($) {
    ...
    var Mobile_Menu = function() {
        $(".site-navigation ul .mega-menu").hide();

        var mmenu = $(".site-navigation#primary-navigation").clone();
        mmenu.attr("id", "mobile-menu").removeClass().appendTo("#site-branding .container");
        mmenu.find('.mega-menu').remove();

        $('<a href="#mobile-menu" class="mmenu-link" id="tb-responsive-nav-trigger"><i class="fa fa-navicon"></i></a>').prependTo("#site-branding .container");
        mmenu.mmenu({
            "offCanvas": {
                "position": "right",
                "zposition": "front"
    ...
    function campaign_section_slides_animation() {
        $(".campaign_section_with_slides .campaign_section_slide").fadeOut(1500);
        $(".campaign_section_with_slides .campaign_section_slide").eq($slideIndex).fadeIn(1500);

        $(".campaign_section_with_slides .slide-title").hide();
        $(".campaign_section_with_slides .slide-title").eq($slideIndex).show();
        setTimeout(campaign_section_slides_animation, 3500);
        $slideIndex +=1;
        if($slideIndex >= $(".campaign_section_with_slides .campaign_section_slide").length){
            $slideIndex = 0;
        }
    }
...
/*
|--------------------------------------------------------------------------
| UItoTop $ Plugin 1.2 by Matt Varone
| http://www.mattvarone.com/web-design/uitotop-$-plugin/
|--------------------------------------------------------------------------
*/
(function($){
    'use strict';
