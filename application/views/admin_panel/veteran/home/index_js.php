...
<script type="text/javascript">
    ...
    function loadData(selectedPage = $selectedPage){
        toggleLoader();
        $sendData = {
            'inputVeteranFullName': $("#inputVeteranFullName").val(),
            'pageIndex': selectedPage
        }
        $.ajax({
            type: 'post',
            url: base_url + 'Veteran/doPagination',
            data: $sendData,
    ...
    $(document).on('click', '.remove-veteran', function () {
        $this = $(this);
            ...
            buttons: {
                specialKey: {
                    ...
                    action: function () {
                        toggleLoader();
                        $sendData = {
                            'inputVeteranId': $this.data('id')
                        }
                        $.ajax({
                            type: 'post',
                            url: base_url + 'Veteran/doDelete',
                            data: $sendData,
                            success: function (data) {
                                toggleLoader();
