...
<script type="text/javascript">
    ...
    function loadData(selectedPage = $selectedPage){
        ...
        $.ajax({
            type: 'post',
            url: base_url + 'Candidate/doInvitePagination',
            data: $sendData,
    ...
    $(document).on('click', '.accept-candidate-invite', function () {
        $this = $(this);
        $title = "<strong class='badge'> " + $this.data('title') + " </strong>";
        $.confirm({
            title: '',
            content: 'آیا از تایید '+ $title +' مطمئن هستید؟',
            buttons: {
                specialKey: {
                    text: 'تایید',
                    btnClass: 'btn-blue',
                    action: function () {
                        ...
                        $.ajax({
                            type: 'post',
                            url: base_url + 'Candidate/doInviteAccept',
                            data: $sendData,
                            success: function (data) {
                                toggleLoader();
    ...
    $(document).on('click', '.remove-candidate-invite', function () {
        $this = $(this);
        $title = "<strong class='badge'> " + $this.data('title') + " </strong>";
        $.confirm({
            title: '',
            ...
            buttons: {
                specialKey: {
                    ...
                    action: function () {
                        ...
                        $.ajax({
                            type: 'post',
                            url: base_url + 'Candidate/doInviteDelete',
                            data: $sendData,
                            success: function (data) {
                                toggleLoader();
                                $result = jQuery.parseJSON(data);
